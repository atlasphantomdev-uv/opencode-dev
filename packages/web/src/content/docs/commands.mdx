...
You can add custom commands by creating markdown files in the `commands/` directory.
...
The `template` is the markdown body of the command file: the prompt sent to the LLM when the command is executed.
...
Use the `description` frontmatter to provide a brief description of what the command does.
...
Use the `agent` frontmatter to optionally specify which [agent](/docs/agents) should execute this command.
...
```markdown title=".opencode/commands/review.md"
---
agent: plan
---
```

This is an **optional** field. If not specified, defaults to your current agent.
...
```markdown title=".opencode/commands/analyze.md"
---
subtask: true
---
```
...
Use the `model` frontmatter to override the default model for this command.
