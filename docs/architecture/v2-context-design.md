...
| MCP                    | `packages/opencode/src/mcp/index.ts:164-200`; V1 clients, tools, instructions, resources     | No native V2 MCP adapter in runner; V2 gap marked in `packages/core/src/session/runner/llm.ts:64-68`                      | Migration gap                                                   |
...
| Diagnostics            | V1 LSP formatter/runtime, `packages/opencode/src/lsp/diagnostic.ts:20-27`                    | No V2 diagnostic context/tool/event boundary                                                                              | Missing V2 capability                                           |
...
1. V2 request assembly combines provider prompt, declaration, and `ContextEpoch` baseline, but does not yet provide full V1 environment, configured instruction, skill-body, MCP instruction, or prompt-expansion parity. Canonical checklist: `specs/v2/session.md:123-151`.
2. V2 tool materialization currently uses core application/local registrations. Runner source explicitly marks policy-filtered MCP, plugin, and structured-output definitions incomplete: `packages/core/src/session/runner/llm.ts:64-68,207-229`.
3. No V2 diagnostic source, tool, or event boundary exists. LSP formatting remains V1-owned: `packages/opencode/src/lsp/diagnostic.ts:20-27`.
