...
| -------------------------- | ------------------------------------------------------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Durable Context Source     | Environment facts and host-local date                                    | complete | None. Model identity is declared per request (`session/system.ts:44`), and the model is resolved per turn rather than held as a stale Location-wide value.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Durable Context Source     | Global and upward project instructions                                   | complete | None. Discovery targets are `AGENTS.md`, `CLAUDE.md`, and `CONTEXT.md`, plus the global config `AGENTS.md` and `~/.claude/CLAUDE.md` (`instruction-context.ts:69,90-91`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| Durable Context Source     | Configured local/glob and remote URL instructions                        | complete | None. Configured local, glob, `~/`, absolute, and remote URL instructions load through one `core/instructions` source with deterministic ordering, unavailable, and removal semantics (`instruction-context.ts:40-49,72-111`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Durable Context Source     | Selected-agent available skill guidance and skill-body loading           | complete | None. Denied skills are already unreachable: guidance is filtered by the selected agent's rules (`skill.ts:30`), body loads re-check (`tool/skill.ts:76`), and a `skill:*` deny removes the one `skill` tool at materialization (`tool/registry.ts:109,128-131`). No per-skill tool definitions exist to remove individually. Open future decision: whether the single `skill` tool is also suppressed when every skill is denied (deferred; would add a registry-to-`SkillV2` dependency).                                                                                                                                                                                                                        |
| Per-turn request assembly  | Placement, selected model, chronological history, and canonical lowering | complete | None.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Per-turn request assembly  | Selected agent, agent prompt, and effective permissions                  | partial  | V2 applies the agent system prompt when the agent defines one (`runner/llm.ts:325-327`) and uses selected-agent permissions for skill guidance and tool authorization (`runner/llm.ts:297`). Still apply effective agent request policy (see Model variants and request settings below).                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Per-turn request assembly  | Provider/model-specific base instructions                                | complete | None. The provider-family baseline is selected per model and used only when the effective agent defines no system prompt (`session/system.ts:16-41`, `runner/llm.ts:325-327`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Prompt/reference expansion | Native template expansion (ordinary `@` mentions are client-owned)       | missing  | No native V2 template expansion exists. V1 command-template substitution is intentionally not ported (`config.md:50`), so core does not re-parse prompt text with the V1 `FILE_REGEX` and must not gain a second `@` parser. Ordinary `@` mentions are already client-owned: clients resolve them and submit structured `Prompt.files`/`Prompt.agents`, and `SessionV2.prompt` only derives mime while preserving the supplied attachments and `source` spans through durable admission (`session.ts:363-389,466-478`). Making those structured sources model-visible is row `:148`; configured `@alias` reference resolution is row `:150`. A future template mechanism must be designed in its owning V2 domain. |
